...
function buyNow() {
    const itemPrice = 5300;
    let quantity = document.getElementById('quantity').value;
    const totalPrice = itemPrice * quantity;
    alert(`The total price for ${quantity} items is ₹${totalPrice.toLocaleString('en-IN')} INR`);
...
function buyNow1(){
    const priceItem = 6100
    // let quantity = document.getElementById('quantity').value;
    alert(`LongeVit & ResVit Bundle 1 item  price ₹${priceItem} INR`)
}


//Ca-AKG (Calcium Alpha-Ketoglutarate) - 500mg, 30 Veg Capsules
function buyNow2(){
    const priceItem = 1399
    // let quantity = document.getElementById('quantity').value;
    alert(`Ca-AKG (Calcium Alpha-Ketoglutarate) - 500mg, 30 Veg Capsules 1 item price ₹${priceItem} INR`)
}
...
function buyNow3(){
    ...
    // let quantity = document.getElementById('quantity').value;
alert(`Boost Metabolism Bundle 1 item price ₹${priceItem} INR`)
}
...
function buyNow4(){
    const priceItem = 900
    // let quantity = document.getElementById('quantity').value;
    alert(`TMG (Trimethyl glycine) 98% Pure - 500mg, 30 Vegan Capsules 1 item  price ₹${priceItem} INR`);
}
...
document.querySelector('.ask-question').addEventListener('click', () => {
...
// Function to cancel the review and hide the form
function cancelReview() {
  const reviewForm = document.getElementById('reviewForm');
  reviewForm.style.display = 'none';  // Hide the form
  clearForm();  // Optional: Clear the form fields when canceled
}
...
function toggleSubscibeForm() {
  // Get the subscribe option container and delivery box elements
  const subscribeDetails = document.querySelector('.subscribe-details');
  const deliveryBox = document.querySelector('.delivery-every');
  const subscriptionBenefits = document.querySelector('.subscription-benefits');

  // Check if the "Subscribe" option is selected
  const isSubscribed = document.getElementById('subscribe').checked;

  // Toggle visibility based on subscription selection
  if (isSubscribed) {
    subscribeDetails.style.display = 'block'; // Show subscription details
    deliveryBox.style.display = 'block';      // Show delivery box
    subscriptionBenefits.style.display = 'block'; // Show benefits
  } else {
    subscribeDetails.style.display = 'none';  // Hide subscription details
    deliveryBox.style.display = 'none';       // Hide delivery box
    subscriptionBenefits.style.display = 'none'; // Hide benefits
  }
}

function hideSubscribeForm() {
  // When "One-Time Purchase" is clicked, hide all subscription-related elements
  const subscribeDetails = document.querySelector('.subscribe-details');
  const deliveryBox = document.querySelector('.delivery-every');
  const subscriptionBenefits = document.querySelector('.subscription-benefits');
...
// Initialize to hide subscription-related sections on page load
