(function() {
    ...
    function toggleContent(element) {
        ...
        const icon = element.querySelector('.accordion-icon, .FAQ-icon, .blended-icon, .blended-icon1');
        const isExpanded = content.classList.contains('show');

        // Close all other sections (accordions, FAQs, blended titles)
        document.querySelectorAll('.accordion-content, .FAQ-content, .blended-content, .blended-content1').forEach(otherContent => {
            if (otherContent !== content) {
                otherContent.classList.remove('show');
                otherContent.style.display = 'none';
                const otherIcon = otherContent.previousElementSibling.querySelector('.accordion-icon, .FAQ-icon, .blended-icon, .blended-icon1');
        ...
        icon.textContent = isExpanded ? '▲' : '▼'; // Arrow direction
    }
