...
(function() {
    ...
    function setupSidebarNav() {
      document.querySelectorAll('.nav-item').forEach(item => {
        item.addEventListener('mouseover', () => {
          ...
          if (infoButton) {
            infoButton.style.right = '0'; // Slide in on hover
          }
        });
        ...
        item.addEventListener('mouseout', () => {
          const infoButton = item.querySelector('.info-button');
          if (infoButton) {
            infoButton.style.right = '-30px'; // Slide out on mouse leave
          }
        });
      });
    }
    ...
    function selectCountry(countryName, countryFlag) {
      document.getElementById("data").innerText = countryName;
      document.getElementById("imgsrc").src = countryFlag;
      toggleDropdown(); // Close dropdown after selection
    }
    
    // Close dropdown if clicked outside
    window.onclick = function(event) {
      if (!event.target.matches('.selectOption')) {
        var dropdowns = document.getElementsByClassName("optionGroup");
