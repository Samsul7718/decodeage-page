(function() {
    ...
    function handleSlide() {
        ...
        if (reviewCards.length === 0) return; // No cards to slide

        const cardWidth = reviewCards[0].offsetWidth + parseInt(window.getComputedStyle(reviewCards[0]).marginRight, 10);
        const visibleCards = 2; // Number of cards to show at once
        const cardCount = reviewCards.length;
        const maxOffset = cardWidth * (cardCount - visibleCards);
        let currentIndex = 0;
        ...
        updateSlider(); // Initialize button states
    }
