...
function handleClickOutside(event) {
    const sidebar = document.getElementById('sidebar');
    const openButton = document.querySelector('[onclick="openSidebar()"]');

    // Check if the click was outside the sidebar and not on the button that opens it
    if (!sidebar.contains(event.target) && !openButton.contains(event.target)) {
        closeSidebar(); // Close the sidebar
    }
}

// Attach the openSidebar function to the button click event
document.querySelector('[onclick="openSidebar()"]').addEventListener('click', openSidebar);
