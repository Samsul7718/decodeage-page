...
function closeSidebar() {
    ...
    sidebar.style.height = '0'; // Ensure the height is also reset
...
function handleClickOutside(event) {
    ...
    if (!sidebar.contains(event.target) && !event.target.closest('[onclick="openSidebar()"]')) {
        closeSidebar(); // Close the sidebar
    }
}
document.querySelector('.btn.btn-light.text-with-icon.gap-1.bold.link-faded-reverse').addEventListener('click', openSidebar);
