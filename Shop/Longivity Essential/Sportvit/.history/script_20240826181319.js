...
const containerWidth = cardWidth * cardCount;
...
prevButton.addEventListener('click', () => {
    if (currentIndex > 0) {
        currentIndex--;
        reviewContainer.style.transform = `translateX(-${currentIndex * cardWidth}px)`;
    }
});

nextButton.addEventListener('click', () => {
    if (currentIndex < cardCount - 3) { // 3 is the number of visible cards
        currentIndex++;
        reviewContainer.style.transform = `translateX(-${currentIndex * cardWidth}px)`;
    }
});
